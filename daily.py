...
p_list=['Moniter','Mouse','Keyboard','Pen Drive','CPU','Power Unit'] # product list
itmvar=tk.StringVar()
itemscombo = ttk.Combobox(second, values=p_list,textvariable=itmvar,width=13,placeholder="items")
itemscombo.grid(row=6,column=1)

qtvar=tk.IntVar()
quantityentry = tk.Entry(second, textvariable=qtvar,width=15,placeholder="qty")
quantityentry.grid(row=7,column=1)



prcvar=tk.DoubleVar()
priceentry= tk.Entry(second, textvariable=prcvar,width=15,placeholder="price")
priceentry.grid(row=8,column=1)
...
crdtvar=tk.IntVar()
creditentry = tk.Entry(second, textvariable=crdtvar,width=15,placeholder="credit")
creditentry.grid(row=9,column=1)
...
#style.theme_use("clam") # set theam to clam
#style.configure("Treeview", background="azure2", 
#                fieldbackground="green", foreground="black",font=font1)
#style.configure('Treeview.Heading', background="PowderBlue") 
# Using treeview widget
tv = ttk.Treeview(second, selectmode ='browse')
tv.grid(row=2,column=0,columnspan=9,rowspan=2,padx=10,pady=10)
# number of columns
tv["columns"] = ("1", "2", "3","4","5","6","7","8")
tv['show'] = 'headings'
tv.column("1", minwidth = 50, anchor ='c') # width & alignment
tv.column("2", minwidth = 250, anchor ='w')
tv.column("3", minwidth = 150, anchor ='c')
tv.column("4", minwidth = 150, anchor ='e')
tv.column("5", minwidth = 150, anchor ='e')

tv.column("6", minwidth = 150, anchor ='e')
tv.column("7", minwidth = 150, anchor ='e')
tv.column("8", minwidth = 150, anchor ='e')

tv.heading("1", text ="date") # Heading text 
tv.heading("2", text ="name")
tv.heading("3", text ="phone")
tv.heading("4", text ="item")  
tv.heading("5", text ="qty")  
tv.heading("6", text ="price")
...
ll1=tk.Label(second,text='Total :',fg='blue',font=font1,anchor='e')
ll1.grid(row=4,column=5)
ll2=tk.Label(second,text='0',fg='blue',font=font1,anchor='e')
ll2.grid(row=4,column=6)
ll3=tk.Label(second,text='wechi :',fg='blue',font=font1,anchor='e')
ll3.grid(row=5,column=5)
ll4=tk.Label(second,text='0',fg='blue',font=font1,anchor='e')
ll4.grid(row=5,column=6)
ll5=tk.Label(second,text='gebi Total :',fg='red',font=font2,anchor='e')
ll5.grid(row=6,column=5)
ll6=tk.Label(second,text='0',fg='red',font=font2,anchor='e')
ll6.grid(row=6,column=6)


##
bb1=tk.Button(second,text='Add',font=font2,command=lambda:add())
bb1.grid(row=4,column=0)
    
bb2=tk.Button(second,text='Delete',state='disabled',command=lambda:delete())
bb2.grid(row=6,column=0)
bb3=tk.Button(second,text='Del All',command=lambda:delall())
bb3.grid(row=7,column=0)
bb4=tk.Button(second,text='Confirm ',font=font2,bg='gray',command=lambda:db())
bb4.grid(row=5,column=0)
...
def add():
    ...
    qtvars=qtvar.get()
    prcvars=prcvar.get()
    crdtvars=crdtvar.get()
    
    total=prcvars*qtvars
    
    date=datetime.datetime.now().strftime("%d-%m-%y || %H-%M-%S-")
    
    
    
    conn = sqlite3.connect("dailyreport.db")
    table = '''CREATE TABLE IF NOT EXISTS daily 
                    (date INT,name TEXT,phone TEXT,item TEXT,quantity INT,price INT,credit INT,total INT)
            '''
    conn.execute(table)
    
    # Insert Data
    value = '''INSERT INTO daily (date, name, phone, 
            item, quantity, price, credit, total) VALUES 
            (?, ?, ?, ?, ?, ?, ?, ?)'''
    
    myvalue = (date,namevars,phonevars,itmvars,qtvars,prcvars,crdtvars,total)
    
    
    cursor = conn.cursor()
    cursor.execute(value, myvalue)
    conn.commit()
    conn.close()
    ...
    global iid
    iid=iid+1  # Serial number to display 
    total=round(qtvar.get()*prcvar.get(),2) # row wise total 
    tv.insert("", 'end',iid=iid, values =(date,namevar.get(),phonevar.get(),itmvar.get(),qtvar.get(),prcvar.get(),crdtvar.get(),total))
    upd(tv)
...
def upd(tv):
    ...
    for child in tv.get_children():
        sub_total=round(sub_total+float(tv.item(child)["values"][7]),2)
        wechi=round(float(wechi +float(tv.item(child)["values"][6]),2))
    ll2.config(text=str(sub_total)) # shows sub total 
#    tax=round(0.1*sub_total,2)  # 10 % tax rate, update here
    ll4.config(text=str(wechi))
    
    total=round(sub_total-wechi)
    
    ll6.config(text=str(total))
    
#    l8.config(text=str(tax))  # tax amount is displayed 
#    total=round(sub_total+tax,2) # tax added to sub total 
#    l10.config(text=str(total))  # Final price is displayed
    itmvar.set('') # reset the combobox 
    qtvar.set("")  # reset quantity to 1
    prcvar.set("") # reset price to 0.0 
    
    
def sel(self):
    bb2.config(state='active') # Delete button is active now 
def delete():
    p_id = tv.selection()[0] # collect selected row id
    tv.delete(p_id)  # remove the selected row from Treeview
    bb2['state']='disabled' # disable the button 
    upd(tv) # Update the total 
def delall():
    for item in tv.get_children():
        tv.delete(item) # remove row 
    #global total
#    total=0
    itmvar.set('') # reset combobox
    qtvar.set(1) # Update quantity to 1 
    prcvar.set(0.0) # Update price to 0.0
    ll2.config(text='0')  # Update display sub total
    ll4.config(text='0')  # Update display for tax
    ll6.config(text='0') # Update display for total
...
#    l_msg.config(text='Bill No:'+str(id)+',Products:'+str(id.rowcount))
    ...
    delall() # reset function 
    
tv.bind("<<TreeviewSelect>>", sel)  # User selection of row
